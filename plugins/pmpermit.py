...
from Fenix.dB import DEVLIST
from Fenix.dB.logusers_db import *
from Fenix.dB.pmpermit_db import *
        ...
        if not is_approved(user.id) and event.text != UND:
            ...
            try:
                ...
                await asst.edit_message(
                    ...
                    buttons=[
                        Button.inline("ᴀᴘᴘʀᴏᴠᴇ ᴘᴍ", data=f"approve_{user.id}"),
                        Button.inline("ʙʟᴏᴄᴋ ᴘᴍ", data=f"block_{user.id}"),
                    ],
                )
            except KeyError:
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    if "PM-Protection" in event.text or "**PM-Protection" in event.text:
                        return
        ...
        if not is_approved(user.id):
            ...
            try:
                await asst.edit_message(
                    ...
                    buttons=[
                        Button.inline("ᴅɪꜱᴀᴘᴘʀᴏᴠᴇ ᴘᴍ", data=f"disapprove_{user.id}"),
                        Button.inline("ʙʟᴏᴄᴋ", data=f"block_{user.id}"),
                    ],
                    parse_mode="html",
                )
            except KeyError:
...
async def unblockpm(event):
    ...
    try:
        await asst.edit_message(
            ...
            buttons=[
                Button.inline("ʙʟᴏᴄᴋ", data=f"block_{user}"),
            ],
        )
    except KeyError:
        _not_approved[user] = await asst.send_message(
...
async def apr_in(event):
    ...
    if not is_approved(uid):
        ...
        await event.edit(
            ...
            buttons=[
                [
                    Button.inline("ᴅɪꜱᴀᴘᴘʀᴏᴠᴇ ᴘᴍ", data=f"disapprove_{uid}"),
                    Button.inline("ʙʟᴏᴄᴋ", data=f"block_{uid}"),
                ],
            ],
            parse_mode="html",
        )
...
async def disapr_in(event):
    ...
    if is_approved(uid):
        ...
        await event.edit(
            ...
            buttons=[
                [
                    Button.inline("ᴀᴘᴘʀᴏᴠᴇ ᴘᴍ", data=f"approve_{uid}"),
                    Button.inline("ʙʟᴏᴄᴋ", data=f"block_{uid}"),
                ],
            ],
            parse_mode="html",
...
async def unblck_in(event):
    ...
    await event.edit(
        ...
        buttons=Button.inline("ʙʟᴏᴄᴋ", data=f"block_{uid}"),
        parse_mode="html",
    )
...
async def _admin_tools(event):
    ...
    await event.edit(
        buttons=[
            [
                Button.inline("ᴀᴘᴘʀᴏᴠᴇ ᴘᴍ", data=f"approve_{chat}"),
                Button.inline("ʙʟᴏᴄᴋ ᴘᴍ", data=f"block_{chat}"),
            ],
