...
async def _(event):
    ...
    file = f"heartless{sys.argv[-1]}.log" if len(sys.argv) > 1 else "heartless.log"
    if opt == "heroku":
        ...
        file = await Carbon(
            file_name="heartless-logs",
            code=code,
            backgroundColor=choice(ATRA_COL),
        )
        await event.reply("**~ UserBot Logs**", file=file)
    elif opt == "open":
        file = open("heartless.log", "r").read()[-4000:]
