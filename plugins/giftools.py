...
async def igif(e):
    ...
    if match == "bw":
        cmd = f'ffmpeg -i "{z}" -vf format=gray heartless.gif -y'
    else:
        cmd = f'ffmpeg -i "{z}" -vf lutyuv="y=negval:u=negval:v=negval" heartless.gif -y'
    try:
        await bash(cmd)
        await e.client.send_file(e.chat_id, "heartless.gif", supports_streaming=True)
        os.remove(z)
