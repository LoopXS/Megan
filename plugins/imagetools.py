...
async def _(event):
    ...
    ret, frame = img.read()
    cv2.imwrite("heartless.jpg", frame)
    ...
    r = requests.post(
        ...
        files={"image": open("heartless.jpg", "rb")},
        headers={"api-key": key},
    )
    os.remove("heartless.jpg")
    os.remove(image)
    if "status" in r.json():
...
async def ult_tools(event):
    ...
    file = await con.convert(ultt, convert_to="png", outname="heartless")
    ult = cv2.imread(file)
    if match == "grey":
    ...
    cv2.imwrite("heartless.jpg", ultroid)
...
async def ultd(event):
    ...
    await download_file(ms, "heartless.png")
    img = Image.open("heartless.png").convert("RGB")
    img.save("heartless.webp", "webp")
    await event.client.send_file(
        event.chat_id,
        "heartless.webp",
        force_document=False,
    ...
    await xx.delete()
    os.remove("heartless.png")
    os.remove("heartless.webp")
    os.remove(ultt)
...
async def pixelator(event):
    ...
    await msg.respond("• Pixelated By @DarkPentesterX", file="output.jpg")
    await msg.delete()
